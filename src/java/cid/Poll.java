...
public final class Poll extends AbstractPoll {
    ...
    private static final boolean isPollsProcessing = Cid.getBooleanProperty("cid.processPolls");
    ...
    public static DbIterator<Poll> getActivePolls(int from, int to) {
        return pollTable.getManyBy(new DbClause.IntClause("finish_height", DbClause.Op.GT, Cid.getBlockchain().getHeight()), from, to);
    }
    ...
    public static DbIterator<Poll> getPollsByAccount(long accountId, boolean includeFinished, boolean finishedOnly, int from, int to) {
        ...
        if (finishedOnly) {
            dbClause = dbClause.and(new DbClause.IntClause("finish_height", DbClause.Op.LTE, Cid.getBlockchain().getHeight()));
        } else if (!includeFinished) {
            dbClause = dbClause.and(new DbClause.IntClause("finish_height", DbClause.Op.GT, Cid.getBlockchain().getHeight()));
        }
    ...
    public static DbIterator<Poll> searchPolls(String query, boolean includeFinished, int from, int to) {
        DbClause dbClause = includeFinished ? DbClause.EMPTY_CLAUSE : new DbClause.IntClause("finish_height", DbClause.Op.GT, Cid.getBlockchain().getHeight());
    ...
    private Poll(Transaction transaction, Attachment.MessagingPollCreation attachment) {
        ...
        this.timestamp = Cid.getBlockchain().getLastBlockTimestamp();
    }
    ...
    public boolean isFinished() {
        return finishHeight <= Cid.getBlockchain().getHeight();
    }

    private List<OptionResult> countResults(VoteWeighting voteWeighting) {
        int countHeight = Math.min(finishHeight, Cid.getBlockchain().getHeight());
        if (countHeight < Cid.getBlockchainProcessor().getMinRollbackHeight()) {
            return null;
        }
