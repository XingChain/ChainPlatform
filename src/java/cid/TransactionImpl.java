...
final class TransactionImpl implements Transaction {
    ...
    static final class BuilderImpl implements Builder {
        ...
        @Override
        public TransactionImpl build(String secretPhrase) throws CidException.NotValidException {
            if (timestamp == Integer.MAX_VALUE) {
                timestamp = Cid.getEpochTime();
            }
            if (!ecBlockSet) {
        ...
        @Override
        public TransactionImpl build() throws CidException.NotValidException {
            return build(null);
        }
    ...
    private TransactionImpl(BuilderImpl builder, String secretPhrase) throws CidException.NotValidException {
        ...
        if (builder.feeNQT <= 0 || (Constants.correctInvalidFees && builder.signature == null)) {
            int effectiveHeight = (height < Integer.MAX_VALUE ? height : Cid.getBlockchain().getHeight());
        ...
        if (builder.signature != null && secretPhrase != null) {
            throw new CidException.NotValidException("Transaction is already signed");
            ...
            if (getSenderPublicKey() != null && ! Arrays.equals(senderPublicKey, Crypto.getPublicKey(secretPhrase))) {
                throw new CidException.NotValidException("Secret phrase doesn't match transaction sender public key");
            }
    ...
    static TransactionImpl.BuilderImpl newTransactionBuilder(byte[] bytes) throws CidException.NotValidException {
        try {
            ...
            if (buffer.hasRemaining()) {
                throw new CidException.NotValidException("Transaction bytes too long, " + buffer.remaining() + " extra bytes");
            }
            return builder;
        } catch (CidException.NotValidException|RuntimeException e) {
            Logger.logDebugMessage("Failed to parse transaction bytes: " + Convert.toHexString(bytes));
            throw e;
        }
    }

    static TransactionImpl.BuilderImpl newTransactionBuilder(byte[] bytes, JSONObject prunableAttachments) throws CidException.NotValidException {
    ...
    static TransactionImpl parseTransaction(JSONObject transactionData) throws CidException.NotValidException {
        ...
        if (transaction.getSignature() != null && !transaction.checkSignature()) {
            throw new CidException.NotValidException("Invalid transaction signature for transaction " + transaction.getJSONObject().toJSONString());
        }
        return transaction;
    }

    static TransactionImpl.BuilderImpl newTransactionBuilder(JSONObject transactionData) throws CidException.NotValidException {
        try {
            ...
            if (transactionType == null) {
                throw new CidException.NotValidException("Invalid transaction type: " + type + ", " + subtype);
            }
    ...
    @Override
    public void validate() throws CidException.ValidationException {
        if (timestamp == 0 ? (deadline != 0 || feeNQT != 0) : (deadline < 1 || feeNQT <= 0)
                || feeNQT > Constants.MAX_BALANCE_NQT
                || amountNQT < 0
                || amountNQT > Constants.MAX_BALANCE_NQT
                || type == null) {
            throw new CidException.NotValidException("Invalid transaction parameters:\n type: " + type + ", timestamp: " + timestamp
        ...
        if (referencedTransactionFullHash != null && referencedTransactionFullHash.length != 32) {
            throw new CidException.NotValidException("Invalid referenced transaction full hash " + Convert.toHexString(referencedTransactionFullHash));
        }

        if (attachment == null || type != attachment.getTransactionType()) {
            throw new CidException.NotValidException("Invalid attachment " + attachment + " for transaction of type " + type);
        }

        if (! type.canHaveRecipient()) {
            if (recipientId != 0 || getAmountNQT() != 0) {
                throw new CidException.NotValidException("Transactions of this type must have recipient == 0, amount == 0");
            }
        }

        if (type.mustHaveRecipient()) {
            if (recipientId == 0) {
                throw new CidException.NotValidException("Transactions of this type must have a valid recipient");
            }
        }
        ...
        for (Appendix.AbstractAppendix appendage : appendages) {
            ...
            if (! appendage.verifyVersion()) {
                throw new CidException.NotValidException("Invalid attachment version " + appendage.getVersion());
            }
        ...
        if (getFullSize() > Constants.MAX_PAYLOAD_LENGTH) {
            throw new CidException.NotValidException("Transaction size " + getFullSize() + " exceeds maximum payload size");
        }
        int blockchainHeight = Cid.getBlockchain().getHeight();
        if (!validatingAtFinish) {
            long minimumFeeNQT = getMinimumFeeNQT(blockchainHeight);
            if (feeNQT < minimumFeeNQT) {
                throw new CidException.NotCurrentlyValidException(String.format("Transaction fee %f %s less than minimum fee %f %s at height %d",
                        ((double) feeNQT) / Constants.ONE_CID, Constants.COIN_SYMBOL, ((double) minimumFeeNQT) / Constants.ONE_CID, Constants.COIN_SYMBOL, blockchainHeight));
            }
            if (ecBlockId != 0) {
                if (blockchainHeight < ecBlockHeight) {
                    throw new CidException.NotCurrentlyValidException("ecBlockHeight " + ecBlockHeight
    ...
    private long getMinimumFeeNQT(int blockchainHeight) {
        ...
        if (referencedTransactionFullHash != null) {
            totalFee = Math.addExact(totalFee, Constants.ONE_CID);
        }
        return totalFee;
    }
