...
public abstract class EntityDbTable<T> extends DerivedDbTable {
    ...
    public void checkAvailable(int height) {
        if (multiversion) {
            int minRollBackHeight = isPersistent() && Cid.getBlockchainProcessor().isScanning() ?
                    Math.max(Cid.getBlockchainProcessor().getInitialScanHeight() - Constants.MAX_ROLLBACK, 0)
                    : Cid.getBlockchainProcessor().getMinRollbackHeight();
    ...
    private boolean doesNotExceed(int height) {
        return Cid.getBlockchain().getHeight() <= height && ! (isPersistent() && Cid.getBlockchainProcessor().isScanning());
    }
