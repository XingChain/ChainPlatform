 ...
 *
 *   java -cp "classes:lib/*:conf" cid.tools.CompactDatabase
 *
 * To run the database compact tool on Windows:
 *
 *   java -cp "classes;lib/*;conf" -Dcid.runtime.mode=desktop cid.tools.CompactDatabase
 */
public class CompactDatabase {
    ...
    public static void main(String[] args) {
        //
        // Initialize Cid properties and logging
        //
        Logger.init();
        //
    ...
    private static int compactDatabase() {
        ...
        //
        String dbPrefix = Constants.isTestnet ? "cid.testDb" : "cid.db";
        String dbType = Cid.getStringProperty(dbPrefix + "Type");
        if (!"h2".equals(dbType)) {
            Logger.logErrorMessage("Database type must be 'h2'");
            return 1;
        }
        String dbUrl = Cid.getStringProperty(dbPrefix + "Url");
        if (dbUrl == null) {
            String dbPath = Cid.getDbDir(Cid.getStringProperty(dbPrefix + "Dir"));
            dbUrl = String.format("jdbc:%s:%s", dbType, dbPath);
        }
        String dbParams = Cid.getStringProperty(dbPrefix + "Params");
        dbUrl += ";" + dbParams;
        if (!dbUrl.contains("MV_STORE=")) {
            dbUrl += ";MV_STORE=FALSE";
        }
        String dbUsername = Cid.getStringProperty(dbPrefix + "Username", "sa");
        String dbPassword = Cid.getStringProperty(dbPrefix + "Password", "sa", true);
        //
        ...
        File dbFile = new File(dbDir, "cid.h2.db");
        if (!dbFile.exists()) {
            dbFile = new File(dbDir, "cid.mv.db");
            if (!dbFile.exists()) {
