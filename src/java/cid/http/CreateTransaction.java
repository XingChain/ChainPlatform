...
import static cid.http.JSONResponses.*;
...
abstract class CreateTransaction extends APIServlet.APIRequestHandler {
    ...
    private Appendix.Phasing parsePhasing(HttpServletRequest req) throws ParameterException {
        int finishHeight = ParameterParser.getInt(req, "phasingFinishHeight",
                Cid.getBlockchain().getHeight() + 1,
                Cid.getBlockchain().getHeight() + Constants.MAX_PHASING_DURATION + 1,
                true);
    ...
    final JSONStreamAware createTransaction(HttpServletRequest req, Account senderAccount, long recipientId,
                                            long amountNQT, Attachment attachment) throws CidException {
        ...
        if (ecBlockId != 0 && ecBlockId != Cid.getBlockchain().getBlockIdAtHeight(ecBlockHeight)) {
            return INCORRECT_EC_BLOCK;
        }
        if (ecBlockId == 0 && ecBlockHeight > 0) {
            ecBlockId = Cid.getBlockchain().getBlockIdAtHeight(ecBlockHeight);
        }
        ...
        try {
            Transaction.Builder builder = Cid.newTransactionBuilder(publicKey, amountNQT, feeNQT,
            ...
            } catch (CidException.NotYetEncryptedException ignore) {}
        ...
        } catch (CidException.NotYetEnabledException e) {
