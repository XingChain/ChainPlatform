...
import cid.AccountLedger;
import cid.AccountLedger.LedgerEntry;
import cid.Block;
import cid.BlockchainProcessor;
import cid.Db;
import cid.Cid;
import cid.Transaction;
import cid.TransactionProcessor;
import cid.db.TransactionalDb;
import cid.peer.Peer;
 ...
 * has not been received within cid.apiEventTimeout seconds.
 *
 * The maximum number of event users is specified by cid.apiMaxEventUsers.
 */
class EventListener implements Runnable, AsyncListener, TransactionalDb.TransactionCallback {

    /** Maximum event users */
    static final int maxEventUsers = Cid.getIntProperty("cid.apiMaxEventUsers");

    /** Event registration timeout (seconds) */
    static final int eventTimeout = Math.max(Cid.getIntProperty("cid.apiEventTimeout"), 15);

    /** Blockchain processor */
    static final BlockchainProcessor blockchainProcessor = Cid.getBlockchainProcessor();

    /** Transaction processor */
    static final TransactionProcessor transactionProcessor = Cid.getTransactionProcessor();
    ...
    /** Cid event listeners */
    private final List<CidEventListener> cidEventListeners = new ArrayList<>();
     ...
     *
     * Cid event listeners will be added for the specified events
     *
     * @param   eventRegistrations      List of Cid event registrations
     ...
     *
     * @param   eventRegistrations      Cid event registrations
     * @throws  EventListenerException  Invalid Cid event
     */
    void addEvents(List<EventRegistration> eventRegistrations) throws EventListenerException {
        ...
        try {
            ...
            for (EventRegistration event : eventRegistrations) {
                ...
                Iterator<CidEventListener> it = cidEventListeners.iterator();
                while (it.hasNext()) {
                    CidEventListener listener = it.next();
                ...
                if (addListener) {
                    CidEventListener listener = new CidEventListener(event);
                    listener.addListener();
                    cidEventListeners.add(listener);
                }
            }
        } finally {
            lock.unlock();
    ...
    void deactivateListener() {
        ...
        try {
            ...
            //
            cidEventListeners.forEach(CidEventListener::removeListener);
        } finally {
            lock.unlock();
        }
     ...
     * Cid event listener
     */
    private class CidEventListener {

        /** Event handler */
        private final CidEventHandler eventHandler;

        /**
         * Create the Cid event listener
         *
         * @param   eventRegistration           Event registration
         * @throws  EventListenerException      Invalid event
         */
        public CidEventListener(EventRegistration eventRegistration) throws EventListenerException {
        ...
        /**
         * Add the Cid listener for this event
         */
        ...
        /**
         * Remove the Cid listener for this event
         */
        ...
        /**
         * Return the hash code for this Cid event listener
         *
         * @return                  Hash code
         */
        @Override
        ...
        /**
         * Check if two Cid events listeners are equal
         *
        ...
        public boolean equals(Object obj) {
            return (obj != null && (obj instanceof CidEventListener) &&
                    eventHandler.equals(((CidEventListener)obj).eventHandler));
        }

        /**
         * Cid listener event handler
         */
        private abstract class CidEventHandler {
            ...
            /**
             * Create the Cid event handler
             *
             * @param   eventRegistration   Event registration
             */
            public CidEventHandler(EventRegistration eventRegistration) {
            ...
            @Override
            public boolean equals(Object obj) {
                return (obj != null && (obj instanceof CidEventHandler) &&
                                        owner == ((CidEventHandler)obj).owner &&
                                        accountId == ((CidEventHandler)obj).accountId &&
                                        event == ((CidEventHandler)obj).event);
            }
        }
         ...
         * Peer event handler
         */
        private class PeerEventHandler extends CidEventHandler implements Listener<Peer> {
         ...
         */
        private class BlockEventHandler extends CidEventHandler implements Listener<Block> {
         ...
         */
        private class TransactionEventHandler extends CidEventHandler implements Listener<List<? extends Transaction>> {
         ...
         */
        private class LedgerEventHandler extends CidEventHandler implements Listener<LedgerEntry> {
    ...
    static class EventRegistration {
         ...
         *
         * @param   event           Cid listener event
        ...
        /**
         * Return the Cid listener event
         *
         * @return                  Cid listener event
         */
        public Enum<? extends Enum> getEvent() {
            return event;
        }
