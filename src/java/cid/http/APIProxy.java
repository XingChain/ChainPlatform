...
public class APIProxy {
    ...
    static final boolean enableAPIProxy = Constants.isLightClient ||
            (Cid.getBooleanProperty("cid.enableAPIProxy") && ! API.isOpenAPI);
    private static final int blacklistingPeriod = Cid.getIntProperty("cid.apiProxyBlacklistingPeriod") / 1000;
    static final String forcedServerURL = Cid.getStringProperty("cid.forceAPIProxyServerURL", "");
    ...
    static boolean isActivated() {
        return Constants.isLightClient || (enableAPIProxy && Cid.getBlockchainProcessor().isDownloading());
    }
    ...
    boolean blacklistHost(String host) {
        ...
        blacklistedPeers.put(host, Cid.getEpochTime() + blacklistingPeriod);
