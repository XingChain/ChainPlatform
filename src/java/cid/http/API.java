...
import static cid.http.JSONResponses.MISSING_ADMIN_PASSWORD;
import static cid.http.JSONResponses.INCORRECT_ADMIN_PASSWORD;
import static cid.http.JSONResponses.LOCKED_ADMIN_PASSWORD;
import static cid.http.JSONResponses.NO_PASSWORD_IN_CONFIG;

public final class API {

    public static final int TESTNET_API_PORT = 6789;
    ...
    public static final String adminPassword = Cid.getStringProperty("cid.adminPassword", "", true);
    static final boolean disableAdminPassword;
    static final int maxRecords = Cid.getIntProperty("cid.maxAPIRecords");
    static final boolean enableAPIUPnP = Cid.getBooleanProperty("cid.enableAPIUPnP");
    public static final int apiServerIdleTimeout = Cid.getIntProperty("cid.apiServerIdleTimeout");
    public static final boolean apiServerCORS = Cid.getBooleanProperty("cid.apiServerCORS");
    private static final String forwardedForHeader = Cid.getStringProperty("cid.forwardedForHeader");
    ...
    static {
        List<String> disabled = new ArrayList<>(Cid.getStringListProperty("cid.disabledAPIs"));
        Collections.sort(disabled);
        disabledAPIs = Collections.unmodifiableList(disabled);
        disabled = Cid.getStringListProperty("cid.disabledAPITags");
        Collections.sort(disabled);
        ...
        List<String> allowedBotHostsList = Cid.getStringListProperty("cid.allowedBotHosts");
        ...
        boolean enableAPIServer = Cid.getBooleanProperty("cid.enableAPIServer");
        if (enableAPIServer) {
            final int port = Constants.isTestnet ? TESTNET_API_PORT : Cid.getIntProperty("cid.apiServerPort");
            final int sslPort = Constants.isTestnet ? TESTNET_API_SSLPORT : Cid.getIntProperty("cid.apiServerSSLPort");
            final String host = Cid.getStringProperty("cid.apiServerHost");
            disableAdminPassword = Cid.getBooleanProperty("cid.disableAdminPassword") || ("127.0.0.1".equals(host) && adminPassword.isEmpty());

            apiServer = new Server();
            ServerConnector connector;
            boolean enableSSL = Cid.getBooleanProperty("cid.apiSSL");
            //
            ...
            if (enableSSL) {
                ...
                String keyStorePath = Paths.get(Cid.getUserHomeDir()).resolve(Paths.get(Cid.getStringProperty("cid.keyStorePath"))).toString();
                Logger.logInfoMessage("Using keystore: " + keyStorePath);
                sslContextFactory.setKeyStorePath(keyStorePath);
                sslContextFactory.setKeyStorePassword(Cid.getStringProperty("cid.keyStorePassword", null, true));
                ...
                sslContextFactory.setKeyStoreType(Cid.getStringProperty("cid.keyStoreType"));
                List<String> ciphers = Cid.getStringListProperty("cid.apiSSLCiphers");
                if (!ciphers.isEmpty()) {
            ...
            String apiResourceBase = Cid.getStringProperty("cid.apiResourceBase");
            if (apiResourceBase != null) {
                ...
                apiHandler.setWelcomeFiles(new String[]{Cid.getStringProperty("cid.apiWelcomeFile")});
            }

            String javadocResourceBase = Cid.getStringProperty("cid.javadocResourceBase");
            ...
            ServletHolder servletHolder = apiHandler.addServlet(APIServlet.class, "/cid");
            servletHolder.getRegistration().setMultipartConfig(new MultipartConfigElement(
                    null, Math.max(Cid.getIntProperty("cid.maxUploadFileSize"), Constants.MAX_TAGGED_DATA_DATA_LENGTH), -1L, 0));

            servletHolder = apiHandler.addServlet(APIProxyServlet.class, "/cid-proxy");
            ...
            if (!Cid.getBooleanProperty("cid.enableAPIServerGZIPFilter", isOpenAPI)) {
                gzipHandler.setExcludedPaths("/cid", "/cid-proxy");
            }
            gzipHandler.setIncludedMethods("GET", "POST");
            gzipHandler.setMinGzipSize(cid.peer.Peers.MIN_COMPRESS_SIZE);
            ...
            if (Cid.getBooleanProperty("cid.apiFrameOptionsSameOrigin")) {
