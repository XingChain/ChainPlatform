...
public final class Shuffling {
    ...
    private static final boolean deleteFinished = Cid.getBooleanProperty("cid.deleteFinishedShufflings");
    ...
    private void cancel(Block block) {
        ...
        if (blamedAccountId != 0) {
            ...
            for (int i = 0; i < 3; i++) {
                ...
                Logger.logDebugMessage("Shuffling penalty %f %s awarded to forger at height %d", ((double)fee) / Constants.ONE_CID, Constants.COIN_SYMBOL, block.getHeight() - i - 1);
            }
            ...
            Logger.logDebugMessage("Shuffling penalty %f %s awarded to forger at height %d", ((double)fee) / Constants.ONE_CID, Constants.COIN_SYMBOL, block.getHeight());
        }
