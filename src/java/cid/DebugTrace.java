...
public final class DebugTrace {
    ...
    static final String QUOTE = Cid.getStringProperty("cid.debugTraceQuote", "\"");
    static final String SEPARATOR = Cid.getStringProperty("cid.debugTraceSeparator", "\t");
    static final boolean LOG_UNCONFIRMED = Cid.getBooleanProperty("cid.debugLogUnconfirmed");

    static void init() {
        List<String> accountIdStrings = Cid.getStringListProperty("cid.debugTraceAccounts");
        String logName = Cid.getStringProperty("cid.debugTraceLog");
        ...
        Cid.getBlockchainProcessor().addListener(block -> debugTrace.resetLog(), BlockchainProcessor.Event.RESCAN_BEGIN);
    ...
    public static DebugTrace addDebugTrace(Set<Long> accountIds, String logName) {
        ...
        Cid.getBlockchainProcessor().addListener(debugTrace::traceBeforeAccept, BlockchainProcessor.Event.BEFORE_BLOCK_ACCEPT);
        Cid.getBlockchainProcessor().addListener(debugTrace::trace, BlockchainProcessor.Event.BEFORE_BLOCK_APPLY);
        Cid.getTransactionProcessor().addListener(transactions -> debugTrace.traceRelease(transactions.get(0)), TransactionProcessor.Event.RELEASE_PHASED_TRANSACTION);
    ...
    private void traceShufflingCancel(Shuffling shuffling) {
        ...
        if (blamedAccountId != 0 && include(blamedAccountId)) {
            ...
            long generatorId = Cid.getBlockchain().getLastBlock().getGeneratorId();
            if (include(generatorId)) {
                Map<String,String> generatorMap = getValues(generatorId, false);
                generatorMap.put("generation fee", String.valueOf(fee));
                generatorMap.put("event", "shuffling blame");
    ...
    private Map<String,String> lessorGuaranteedBalance(Account account, long lesseeId) {
        ...
        map.put("timestamp", String.valueOf(Cid.getBlockchain().getLastBlock().getTimestamp()));
        map.put("height", String.valueOf(Cid.getBlockchain().getHeight()));
    ...
    private Map<String,String> getValues(long accountId, Block block) {
        ...
        if (block.getHeight() > 3) {
            ...
            for (int i = 0; i < backFees.length; i++) {
                ...
                if (include(previousGeneratorId)) {
                    ...
                    map.put("effective balance", String.valueOf(Account.getAccount(previousGeneratorId).getEffectiveBalanceCID()));
        ...
        Map<String,String> map = getValues(accountId, false);
        map.put("effective balance", String.valueOf(Account.getAccount(accountId).getEffectiveBalanceCID()));
