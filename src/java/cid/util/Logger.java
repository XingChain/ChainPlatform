...
/**
 * Handle logging for the Cid node server
 */
public final class Logger {
     ...
     * The cid/conf/logging-default.properties and cid/conf/logging.properties configuration
    ...
    static {
        ...
        System.setProperty("java.util.logging.manager", "cid.util.CidLogManager");
        if (!(LogManager.getLogManager() instanceof CidLogManager)) {
            System.setProperty("java.util.logging.manager",
                    (oldManager != null ? oldManager : "java.util.logging.LogManager"));
        }
        if (! Boolean.getBoolean("cid.doNotConfigureLogging")) {
            try {
                Properties loggingProperties = new Properties();
                Cid.loadProperties(loggingProperties, "logging-default.properties", true);
                Cid.loadProperties(loggingProperties, "logging.properties", false);
                Cid.updateLogFileHandler(loggingProperties);
        ...
        log = org.slf4j.LoggerFactory.getLogger(cid.Cid.class);
        enableStackTraces = Cid.getBooleanProperty("cid.enableStackTraces");
        enableLogTraceback = Cid.getBooleanProperty("cid.enableLogTraceback");
    ...
    public static void shutdown() {
        if (LogManager.getLogManager() instanceof CidLogManager) {
            ((CidLogManager) LogManager.getLogManager()).cidShutdown();
        }
    }
