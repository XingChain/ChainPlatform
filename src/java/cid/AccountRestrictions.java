...
import cid.CidException.AccountControlException;
...
public final class AccountRestrictions {
    ...
    public static final class PhasingOnly {
        ...
        private void checkTransaction(Transaction transaction) throws AccountControlException {
            if (maxFees > 0 && Math.addExact(transaction.getFeeNQT(), PhasingPoll.getSenderPhasedTransactionFees(transaction.getSenderId())) > maxFees) {
                throw new AccountControlException(String.format("Maximum total fees limit of %f %s exceeded", ((double)maxFees)/Constants.ONE_CID, Constants.COIN_SYMBOL));
            }
            ...
            int duration = phasingAppendix.getFinishHeight() - Cid.getBlockchain().getHeight();
    ...
    static void checkTransaction(Transaction transaction) throws CidException.NotCurrentlyValidException {
        Account senderAccount = Account.getAccount(transaction.getSenderId());
        if (senderAccount == null) {
            throw new CidException.NotCurrentlyValidException("Account " + Long.toUnsignedString(transaction.getSenderId()) + " does not exist yet");
        }
