...
CHANGELOG=cid-clone-client-${VERSION}.changelog.txt
OBFUSCATE=$2
MACVERSION=$3
if [ -x ${MACVERSION} ];
then
...
FILES="${FILES} cid.policy ciddesktop.policy Wallet.url Dockerfile"
...
then
echo obfuscate
~/proguard/proguard5.3.3/bin/proguard.sh @cid.pro
mv ../cid.map ../cid.map.${VERSION}
else
...
/Library/Java/JavaVirtualMachines/jdk1.8.0_162.jdk/Contents/Home/bin/javapackager -deploy -outdir . -outfile ${APPLICATION}-client -name ${APPLICATION}-installer -width 34 -height 43 -native dmg -srcfiles ${PACKAGE}.jar -appclass com.izforge.izpack.installer.bootstrap.Installer -v -Bmac.category=Business -Bmac.CFBundleIdentifier=org.cid.client.installer -Bmac.CFBundleName=${APPLICATION}-Installer -Bmac.CFBundleVersion=${MACVERSION} -BappVersion=${MACVERSION} -Bicon=installer/AppIcon.icns -Bmac.signing-key-developer-id-app="Developer ID Application: Stichting CID (YU63QW5EFW)" > installer/javapackager.log 2>&1
